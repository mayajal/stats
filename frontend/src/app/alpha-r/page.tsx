...
import { FaChartBar, FaAtom } from "react-icons/fa"; // Keeping FaAtom for now, or FaChartBar
                    ...
                    {tables.map((table, tIdx) => {
                        ...
                        if (isParamEstimates) {
                            ...
                            const renderSubTable = (title: string, rowsToRender: string[][], tableHeaders: string[]) => {
                                ...
                                return (
                                    <div key={`sub-table-${title}`} className="space-y-4">
                ...
                {results.plots && (results.plots.mean_bar_plot || results.plots.mean_box_plot) && (
                    <div className="mt-6">
                        ...
                        <div className="space-y-4"> {/* Use space-y for vertical stacking */}
                            {results.plots.mean_bar_plot && (                                <div className="!border !border-primary rounded-lg p-2 w-full"> {/* w-full to ensure full width */}
...
export default function ALPHARPage() { // Changed function name
    ...
    const handleAnalyze = async () => {
        ...
        // --- START: Added client-side validation for required fields ---
        if (!responseCol) { setError("Please select a Response Variable."); setLoading(false); return; }
        if (!tukeyFactor) { setError("Please select a Factor for Tukey's Test."); setLoading(false); return; }
        if (!treatment) { setError("Please select a Treatment variable."); setLoading(false); return; }
        if (!replication) { setError("Please select a Replication variable."); setLoading(false); return; }
        if (!block) { setError("Please select a Block variable."); setLoading(false); return; }
        // --- END: Added client-side validation for required fields ---
        ...
        const processAndFetch = async (jsonData: string) => {
            // --- START: Added validation for jsonData ---
            if (!jsonData || jsonData === "[]") {
                setError("No valid data found after processing. Please check your file and selections.");
                setLoading(false);
                return;
            }
            console.log("Sending jsonData:", jsonData); // ADDED CONSOLE.LOG HERE
            // --- END: Added validation for jsonData ---

            const fixedEffects = [treatment, location].filter(Boolean);
            const randomEffects = [replication, block].filter(Boolean);

            const formData = new FormData();
            formData.append("data_json", jsonData); // Changed to data_json for Plumber
            formData.append("response_col", responseCol);
            
            formData.append("tukey_factor", tukeyFactor);
            formData.append("fixed_effects_str", fixedEffects.join(',')); // Changed to fixed_effects_str
            formData.append("random_effects_str", randomEffects.join(',')); // Changed to random_effects_str

            try {
                // Changed to NEXT_PUBLIC_ALPHA_R_SERVICE_URL
                const serviceUrl = process.env.NEXT_PUBLIC_ALPHA_R_SERVICE_URL || '';
                if (!serviceUrl) throw new Error("ALPHA R service URL is not configured");
            ...
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-8">
                    <div className="flex items-center justify-center mb-4">
                        <FaChartBar style={{ height: '2rem', width: '2rem', color: "#edb83dff", marginRight: '0.75rem' }} /> {/* Changed icon to FaChartBar */}
                        <h1 className="text-3xl font-bold">Alpha Lattice Design (R-based)</h1> {/* Changed title */}
                    </div>
                    <p className="text-muted-foreground">Analyze data with both fixed and random effects using an R-based statistical backend. This model assumes that the effect of a fixed effect (eg. genotype) on response variable (eg. yield) is consistent across all locations. It tries to find the average effect of each fixed effect. This approach is better if you are looking for a genotype that performs well on average across all environments, rather than a specialized one for each location.</p>
                ...
                {dataPreview.length > 0 && (
                    <Card className="mb-8 !border !border-primary rounded-lg">
                        <CardHeader><CardTitle>Data Preview</CardTitle>
                ...
                {dataPreview.length > 0 && (
                    <Card className="mb-8 !border !border-primary rounded-lg">
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <Button onClick={handleAnalyze} disabled={loading || !responseCol || !tukeyFactor || !treatment || !replication || !block} variant="secondary" className="bg-gray-200 text-black-800 font-bold hover:bg-black-300 border border-gray-300">
